...
在实现探测器的响应的时候，最简单的方法是根据平方反比计算（10'）。可以认为 PMT 是一个直径 50cm 的球体，我们的几何文件给出的是球心的位置。
...
同时，考虑到 PMT 的表面是玻璃，可能反射一些光子。因此你应该在模拟时考虑一定概率的反射。我们的基础要求画的图可能不会反映出这一光学过程。你应该考虑一些更明确的表现方式。
