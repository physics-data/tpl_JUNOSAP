...
在实现探测器的响应的时候，最简单的方法是根据平方反比计算（10'）。然而，实际情况中，我们可能需要考虑一次反射的情况。

考虑到一次反射，可能出现的情况主要有全反射（10'）。为了计算全反射，你可以取水的折射率为1.33，液闪的折射率为1.48。对于光学过程的考察，不要求实现细节，但是要求给出 PMT 对于顶点的响应分布。
...
这是模拟程序文件。我提供了一个基础的模板，处理了命令行参数。里面给出了基础的程序框架，但是你可以随意修改。尤其是 `for` 循环是很慢的，如果你擅长使用 `numpy`，也许能矢量化处理。
...
这回我们不考虑一个顶点作为光源，而是一条线段。为了考虑一条线段源在探测器中的响应，你可能需要5个参数：$r,\theta,r',\theta',\phi$。同时，你需要考虑这个线源是一个高速运动的粒子，因此它会产生切伦科夫光。

这一项目不作具体实现要求，请自行探索合理的表现手法，并在报告中写明。

### 加分项目：进阶光学（5'）

上面我们给出了液闪和水的折射率，但是我们没有要求计算折射的情况。这里我们要求计算折射（2'）。

同时，考虑到 PMT 的表面是玻璃，可能反射一些光子。因此你应该在模拟时考虑一定概率的反射（3'）。

我们的基础要求画的图可能不会反映出这些进阶光学过程。你应该考虑一些更明确的表现方式。
